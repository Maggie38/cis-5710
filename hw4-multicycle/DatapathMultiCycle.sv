...
module DatapathMultiCycle (
  ...
  // --- Convenience signals (disassemble to readable string) ---
`ifndef SYNTHESIS
...
`endif
  ...
  // --- CLA Adder (shared for ALU ops, address calc, branch targets) ---
  ...
  // --- Multiplier (sign/zero-extend operands to 33 bits) ---
  ...
  // --- Divider (unsigned core; sign fixup done in execution logic) ---
  wire is_div_instr = insn_div | insn_divu | insn_rem | insn_remu;
  logic [3:0] div_stall_counter;
  logic div_done;
  always_ff @(posedge clk) begin
    if (rst) begin
      div_stall_counter <= 0;
      ...
      if (div_stall_counter == 8) begin
        div_stall_counter <= 0;
      end else begin
        div_stall_counter <= div_stall_counter + 1;
      end
      if (div_stall_counter == 7) begin
        div_done <= 1'b1;
      end else begin
        div_done <= 1'b0;
      end
  ...
  DividerUnsignedPipelined div_unit (
      .clk(clk),
      .rst(rst),
      .stall(1'b0),  // Divider does not need to stall itself yet
      .i_dividend(dividend),
      .i_divisor(divisor),
      .o_quotient(quotient),
      .o_remainder(remainder)
  );
  logic [31:0] quotient_ff, remainder_ff;
  always @(posedge clk) begin
    if (rst) begin
      quotient_ff  <= 0;
      remainder_ff <= 0;
      ...
      remainder_ff <= remainder;
    end
  end
  wire [31:0] rs1_neg = ~rs1_data + 32'd1;
  wire [31:0] rs2_neg = ~rs2_data + 32'd1;
  wire [31:0] neg_quotient = ~quotient_ff + 32'd1;
  wire [31:0] neg_remainder = ~remainder_ff + 32'd1;

  // --- Barrel Shifter (left uses reverse-shift-reverse trick) ---
  ...
  // --- Stall logic ---
  wire  stall = (is_div_instr && !div_done);
  ...
  always_comb begin
    ...
    halt = 1'b0;
    pcNext = stall ? pcCurrent : pc_plus_4;  // Default PC update
    ...
    case (insn_opcode)
      ...
      OpRegReg: begin
        if (insn_add) begin  // Add/Subtract
          ...
          if (rs2_data != 0) begin
            rd_data = (rs1_data[31] ^ rs2_data[31]) ? neg_quotient : quotient_ff;
          end else begin
          ...
          divisor = rs2_data[31] ? rs2_neg : rs2_data;
          if (rs2_data != 0) begin
            rd_data = rs1_data[31] ? neg_remainder : remainder_ff;
          end else begin
            rd_data = rs1_data;
          end
  ...
  assign trace_completed_cycle_status = stall ? CYCLE_DIV : CYCLE_NO_STALL;
